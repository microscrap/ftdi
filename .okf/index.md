...
Package knowledge for `microscrap/ftdi` (libFTDI PHP helpers over **ext-ftdi**, v0.9.0).
...
**Scope:** Document the bindings-only helpers package. Do **not** invent ServiceProviders, GFX, MPSSE protocol layers, or Fabricate remaps here — those belong in peers (`microscrap/mpsse`, `scrapyard-io/framework`, tubes).
**Dist note:** `.okf/` and root `AGENTS.md` are `export-ignore` in `.gitattributes` so Composer dist packages do not ship this bundle.

# Orientation

* [Package (0.9)](orientation/package.md) - Composer identity, namespace, helpers over ext-ftdi.
