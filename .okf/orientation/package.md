...
type: Orientation
title: Package (0.9)
description: "microscrap/ftdi 0.9.0 — libFTDI PHP helpers over ext-ftdi; no ServiceProvider, no intermediate wrapper class."
resource: .
tags: [orientation, ftdi, microscrap, bindings, 0.9]
generated: { by: "cursor-grok-4.6", at: "2026-09-23T23:25:00Z" }
status: draft
sources:
  - id: composer
...
Composer package `microscrap/ftdi` at **0.9.0** — PHP global helpers and VID/PID enums over the [**php-io-extensions/ftdi**](https://github.com/php-io-extensions/ftdi) extension (`ext-ftdi`).[^composer][^readme]
...
| Require | `ext-ftdi` `^0.9.0`[^composer] |
| Suggest | `microscrap/mpsse` `^0.9.0`, `scrapyard-io/framework` `^0.9.0`[^composer] |
...
- Not higher UART/USB adapters — those belong in `scrapyard-io/framework` (suggested peer).[^composer]
...
| Extension | `php-io-extensions/ftdi` 0.9.0 |
